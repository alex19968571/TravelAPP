...
export class PreferenceService {
  ...
  readonly country     = computed(() => COUNTRIES.find(c => c.code === this.countryCode())     ?? COUNTRIES[0]);
  readonly homeCountry = computed(() => COUNTRIES.find(c => c.code === this.homeCountryCode()) ?? COUNTRIES[0]);
  readonly color       = computed(() => COLOR_OPTIONS.find(c => c.id === this.colorId())       ?? COLOR_OPTIONS[0]);
  ...
  constructor() {
    ...
    // 套用 UI 語系至 <html lang="">
    effect(() => {
      document.documentElement.lang = this.homeCountry().locale;
    });

    setInterval(() => this._now.set(new Date()), 1000);
  }

  setTheme(t: Theme): void        { this.theme.set(t);            localStorage.setItem('pref_theme', t); }
  setColor(id: string): void      { this.colorId.set(id);         localStorage.setItem('pref_color', id); }
  setFontSize(s: FontSize): void  { this.fontSize.set(s);         localStorage.setItem('pref_fontSize', s); }
