...
import {
  PreferenceService, COLOR_OPTIONS, COUNTRIES, Country,
...
@Component({
    ...
    <div class="page-container">
      ...
      <div class="card">
        ...
        <div class="theme-grid">
          <button class="theme-card" [class.selected]="pref.theme() === 'light'" (click)="pref.setTheme('light')">
          ...
          <button class="theme-card" [class.selected]="pref.theme() === 'dark'" (click)="pref.setTheme('dark')">
      ...
      <div class="card">
        ...
        <div class="color-grid">
          @for (c of colors; track c.id) {
            <button class="color-swatch" [class.selected]="pref.colorId() === c.id"
                    [style.background]="c.accent" [title]="c.label" (click)="pref.setColor(c.id)">
              @if (pref.colorId() === c.id) { <span class="check">✓</span> }
            </button>
          }
        </div>
      ...
      <div class="card">
        <h3>🏠 當前所在國家</h3>
        <p class="section-desc">影響 UI 語系與幣別換算基準</p>

        <div class="country-picker-inline" [class.open]="showHomePicker()">
          <button class="country-trigger" (click)="toggleHomePicker($event)">
            <span class="fi fi-{{ pref.homeCountry().code.toLowerCase() }}"></span>
            <span class="cname">{{ pref.homeCountry().nativeName }}</span>
            <span class="currency-badge">{{ pref.homeCountry().currency }}</span>
            <span class="caret" [class.flipped]="showHomePicker()">▾</span>
          </button>
  ...
  styles: [`
    ...
    .section-desc { margin: -0.75rem 0 1rem; color: var(--text-secondary); font-size: 0.85rem; }
    ...
    .theme-preview {
      ...
      display: flex; flex-direction: column; gap: 4px; padding: 8px; height: 80px;
    }
    .light-preview { background: #f5f6fa; }
    .dark-preview  { background: #0e0e1a; }
    .preview-bar   { display: block; height: 10px; border-radius: 4px; width: 100%; background: var(--accent); }
    ...
    .color-swatch {
      ...
      transition: transform 0.15s, border-color 0.15s; box-shadow: 0 2px 8px rgba(0,0,0,0.2);
    }
    ...
    .font-btn {
      ...
      cursor: pointer; color: var(--text-primary); font-weight: 500; transition: border-color 0.2s;
    }
    ...
    .country-picker-inline.open .country-trigger { border-color: var(--accent); border-bottom-left-radius: 0; border-bottom-right-radius: 0; }
    .fi { width: 1.4em; flex-shrink: 0; border-radius: 2px; }
    .cname { flex: 1; font-weight: 500; }
    .currency-badge {
      font-size: 0.75rem; padding: 0.15rem 0.5rem; border-radius: 6px;
      background: var(--accent-light); color: var(--accent); font-weight: 600;
    }
    .caret { font-size: 0.7rem; color: var(--text-secondary); transition: transform 0.2s; margin-left: auto; }
    ...
    .country-option {
      ...
      color: var(--text-primary); font-size: 0.9rem; transition: background 0.15s;
    }
    ...
    .country-option:last-child { border-bottom-left-radius: 10px; border-bottom-right-radius: 10px; }
...
export class SettingsComponent {
  ...
  countries = COUNTRIES;
  showHomePicker = signal(false);

  @HostListener('document:click', ['$event'])
  onDocumentClick(e: MouseEvent): void {
    const el = document.querySelector('.country-picker-inline');
    if (el && !el.contains(e.target as Node)) {
      this.showHomePicker.set(false);
    }
  }

  toggleHomePicker(e: MouseEvent): void {
    e.stopPropagation();
    this.showHomePicker.set(!this.showHomePicker());
  }

  selectHome(c: Country): void {
