...
import {
  UserProfileService, PRESET_AVATARS, PresetAvatar, parseAvatar,
...
@Component({
          ...
          <div class="modal-card" (click)="$event.stopPropagation()">
            ...
            <div class="preset-grid">
              @for (p of presetAvatars; track p.id) {
                <button class="preset-swatch" [style.background]="p.bg" (click)="selectPreset(p)">{{ p.emoji }}</button>
              }
            </div>
          </div>
        </div>
  ...
  `,
  styles: [`
    .page-container { max-width: 500px; margin: 0 auto; padding: 1.5rem; background: var(--bg); min-height: 100vh; }
    h1 { font-size: 1.6rem; font-weight: 700; color: var(--text-primary); margin: 0 0 1.5rem; text-align: center; }

    .avatar-hero { display: flex; flex-direction: column; align-items: center; gap: 0.75rem; margin-bottom: 1.5rem; }
    .avatar-big {
      width: 96px; height: 96px; border-radius: 50%; padding: 4px; box-sizing: border-box;
      border: none; background: var(--surface); box-shadow: 0 4px 20px var(--shadow);
    ...
    .account-name { color: var(--text-secondary); font-size: 0.9rem; }

    .card { background: var(--surface); border-radius: 16px; box-shadow: 0 4px 20px var(--shadow); overflow: hidden; }
    .row-item {
      display: flex; align-items: center; justify-content: space-between; gap: 0.75rem;
      width: 100%; padding: 1rem 1.25rem; border: none; background: transparent; cursor: pointer;
      text-align: left; color: var(--text-primary); font-size: 0.95rem;
      border-bottom: 1px solid var(--border);
    }
    .row-item:last-child { border-bottom: none; }
    .row-item:hover { background: var(--accent-light); }
    .row-item.danger { color: #e53e3e; }
    .row-item.danger:hover { background: #fff0f0; }
    .chevron { color: var(--text-secondary); }

    .country-row { cursor: default; }
    .country-row:hover { background: transparent; }
    .row-label { flex-shrink: 0; }
    .country-picker-inline { position: relative; flex: 1; max-width: 200px; }
    .country-trigger {
      display: flex; align-items: center; gap: 0.4rem; width: 100%;
      background: var(--bg); color: var(--text-primary);
      border: 1.5px solid var(--border); border-radius: 10px;
      padding: 0.4rem 0.6rem; cursor: pointer; font-size: 0.85rem; white-space: nowrap;
    }
    .country-trigger:hover, .country-picker-inline.open .country-trigger { border-color: var(--accent); }
    .fi { width: 1.2em; flex-shrink: 0; border-radius: 2px; }
    .cname { flex: 1; font-weight: 500; overflow: hidden; text-overflow: ellipsis; }
    .currency-badge {
      font-size: 0.7rem; padding: 0.1rem 0.4rem; border-radius: 5px;
      background: var(--accent-light); color: var(--accent); font-weight: 700;
    }
    .caret { font-size: 0.65rem; color: var(--text-secondary); transition: transform 0.2s; }
    ...
    .country-picker-inline.open .country-dropdown { display: block; }
    .country-option {
      display: flex; align-items: center; gap: 0.6rem; width: 100%;
      padding: 0.625rem 1rem; border: none; background: transparent; cursor: pointer;
      text-align: left; color: var(--text-primary); font-size: 0.875rem;
    }
    ...
    .modal-card {
      ...
      max-width: 360px; width: 100%; box-shadow: 0 12px 40px var(--shadow);
    }
    ...
    .section-desc { margin: 1rem 0 0; color: var(--text-secondary); font-size: 0.85rem; }
    .preset-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 0.75rem; margin-top: 0.75rem; }
    .preset-swatch {
