...
@Component({
        ...
        <div class="member-list">
          @for (m of members(); track m.id) {
            <div class="member-row-wrap">
              <button class="swipe-delete" (click)="removeMember(m.id)">{{ 'common.delete' | transloco }}</button>
      ...
      @if (inviteModalRole(); as role) {
        <div class="modal-backdrop" (click)="closeInviteModal()">
          <div class="modal-card" (click)="$event.stopPropagation()">
            <h3>{{ (role === 'EDITOR' ? 'tripDetail.inviteEditor' : 'tripDetail.inviteViewer') | transloco }}</h3>

            <div class="modal-row">
              <label>{{ 'trips.inviteCode' | transloco }}</label>
              <div class="modal-value-row">
                <code>{{ role === 'EDITOR' ? trip()?.invite_code_editor : trip()?.invite_code_viewer }}</code>
                <button class="btn-sm" (click)="copy(role === 'EDITOR' ? trip()?.invite_code_editor : trip()?.invite_code_viewer, 'code')">
            ...
            <div class="modal-row">
              ...
              <div class="modal-value-row">
                <span class="link-text">{{ inviteLink(role === 'EDITOR' ? trip()?.invite_code_editor : trip()?.invite_code_viewer) }}</span>
                <button class="btn-sm" (click)="copy(inviteLink(role === 'EDITOR' ? trip()?.invite_code_editor : trip()?.invite_code_viewer), 'link')">
                  {{ (copied() === 'link' ? 'tripDetail.copied' : 'tripDetail.copyLink') | transloco }}
                </button>
              </div>
            </div>

            <button class="btn-secondary full-width" (click)="closeInviteModal()">{{ 'common.confirm' | transloco }}</button>
          </div>
        </div>
      }
    </div>
  ...
  styles: [`
    ...
    .card h3 { margin: 0; color: var(--text-primary); }
    ...
    .invite-dropdown {
      ...
      min-width: 180px; z-index: 100; display: none; padding: 0.4rem;
    }
    .invite-menu.open .invite-dropdown { display: block; }
    .invite-option {
      display: flex; align-items: center; gap: 0.5rem; width: 100%;
      padding: 0.625rem 0.75rem; border: none; background: transparent; cursor: pointer;
      text-align: left; color: var(--text-primary); font-size: 0.875rem; border-radius: 8px;
    }
    .invite-option:hover { background: var(--accent-light); }

    /* 成員清單 */
    .member-list { display: flex; flex-direction: column; gap: 0.75rem; }
    .member-row-wrap { position: relative; overflow: hidden; border-radius: 10px; }
    .swipe-delete {
      position: absolute; top: 0; right: 0; bottom: 0; width: 72px;
      background: #e53e3e; color: white; border: none; font-size: 0.85rem; font-weight: 600;
      cursor: pointer; display: none;
    }
    .member-row {
      display: flex; align-items: center; gap: 0.75rem;
      background: var(--surface); position: relative; transition: transform 0.2s ease;
    }
    .member-avatar { width: 36px; height: 36px; border-radius: 50%; background: var(--accent);
    ...
    @media (hover: none) and (pointer: coarse) {
      ...
      .swipe-delete { display: block; }
      .member-row.swiped { transform: translateX(-72px); }
    }
    ...
    .modal-row label { display: block; font-size: 0.8rem; color: var(--text-secondary); margin-bottom: 0.35rem; }
    .modal-value-row { display: flex; align-items: center; gap: 0.5rem; }
    .modal-value-row code {
      font-family: monospace; font-size: 1rem; font-weight: 700; letter-spacing: 0.06em;
      background: var(--accent-light); color: var(--accent); padding: 0.3rem 0.6rem; border-radius: 8px;
      flex-shrink: 0;
    }
    .link-text {
    ...
    .btn-sm { background: var(--accent-light); color: var(--accent); border: none; border-radius: 8px;
      padding: 0.375rem 0.75rem; cursor: pointer; font-size: 0.8rem; font-weight: 500; white-space: nowrap; flex-shrink: 0; }
